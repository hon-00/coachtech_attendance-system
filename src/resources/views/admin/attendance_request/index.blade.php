...
<div class="content">
    ...
    <div class="tab-group">
        <ul class="tab-list">
            <li class="tab-item">
                <a href="{{ route('admin.attendance_request.index', ['tab' => 'pending']) }}" class="tab-link {{ $tab === 'pending' ? 'active' : '' }}">
                    承認待ち
                </a>
            </li>
            <li class="tab-item">
                <a href="{{ route('admin.attendance_request.index', ['tab' => 'approved']) }}" class="tab-link {{ $tab === 'approved' ? 'active' : '' }}">
                    承認済み
                </a>
            </li>
        </ul>
        ...
        @elseif($tab === 'approved')
            ...
            @else
                <table class="content-table">
                    ...
                    <tbody>
                        @foreach($approvedAdminRequests as $request)
                        <tr class="content-table__row">
                            <td class="content-table__cell--status">{{ $request->status_label }}</td>
                            <td class="content-table__cell--name">{{ $request->user->name }}</td>
                            <td class="content-table__cell--date">
